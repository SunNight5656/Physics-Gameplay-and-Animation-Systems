...
private func RFC_ScheduleTumble_NoRaycast(
  ...
  let globalCfg: RFCConfig = RFC.Cfg();
  if globalCfg.vanillaMode { return; };
  if puppet == null || RFC_TimeDilationBlocksImpulses(puppet, globalCfg) { return; };
...
private func RFC_ScheduleSideTumble_Torque(
  ...
  while i < steps {
    ...
    // ─────────────────────────
    // A one-step directional tumble must fire at full requested strength.
    // The old 0 / Max(1, 0) calculation produced a zero-strength event.
    let tNorm: Float = steps <= 1 ? 1.0 : Cast<Float>(i) / Cast<Float>(steps - 1);
...
let n: Float = steps <= 1 ? 1.0 : Cast<Float>(i) / Cast<Float>(steps - 1);
