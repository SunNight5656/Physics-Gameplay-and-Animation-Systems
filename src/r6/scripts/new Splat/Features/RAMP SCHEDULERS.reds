...
private func RFC_ApplyGravityBurst(
  ...
  let burstCfg: RFCConfig = RFC.Cfg();
  if burstCfg.vanillaMode || !IsDefined(p) || RFC_TimeDilationBlocksImpulses(p, burstCfg) || radius <= 0.0 {
    return;
  }
...
public func RFC_Burst(
  ...
  if !IsDefined(ds) || !IsDefined(p) || RFC_TimeDilationBlocksImpulses(p, c) {
    return;
  }
  if radius <= 0.0 {
    return;
