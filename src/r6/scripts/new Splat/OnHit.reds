...
// SPLAT ONHIT CUTOFF TEST V1714 - immediate vanilla start + slider-only timed cutoff
...
// V1714 ordinary live hit-animation cutoff. Serial prevents an older cutoff
// event from terminating a newer bullet reaction.
@addField(NPCPuppet) private let m_RFC_HitAnimSerial: Int32;
...
private func RFC_ScheduleOrdinaryHitReactionCut(p: wref<NPCPuppet>, serial: Int32, cfg: RFCConfig) -> Void {
  if !IsDefined(p)
    || cfg.hitReactionCutoffDelay <= 0.001
    || cfg.hitReactionsDisabled
    || p.IsDead()
    || RFC_IsIncapacitatedTransition(p) {
    return;
  }

  let cutEvt: ref<RFC_HitReactionCutEvent> = new RFC_HitReactionCutEvent();
  cutEvt.serial = serial;
  let cutDS: ref<DelaySystem> = GameInstance.GetDelaySystem(p.GetGame());
  if IsDefined(cutDS) {
    // IMPORTANT: this duration begins when the reaction animation begins, not
    // when the bullet originally landed.
    cutDS.DelayEvent(p, cutEvt, RFC_ClampF(cfg.hitReactionCutoffDelay, 0.0, 3.0), false);
  } else {
    p.QueueEvent(cutEvt);
  }
}
...
private func OnHitAnimation(hitEvent: ref<gameHitEvent>) -> Void {
  ...
  // Selected Vanilla weapon lanes remain fully native and immediate.
  let hitAnimLane: Int32 = RFC_VanillaWeaponLaneForAttack(hitEvent.attackData, cfg);
  if !cfg.vanillaMode
    && (
      RFC_VanillaWeaponReactionArmed(this)
      || (hitAnimLane > 0 && RFC_VanillaWeaponLaneEnabled(hitAnimLane, cfg))
    ) {
    wrappedMethod(hitEvent);
    return;
  }

  let ordinaryPlayerBullet: Bool =
    !cfg.vanillaMode
    && !this.IsDead()
  ...
  if ordinaryPlayerBullet {
    ...
    wrappedMethod(hitEvent);
    RFC_ScheduleOrdinaryHitReactionCut(this, serial, cfg);
    return;
  }
...
private func RFC_StopOrdinaryHitReaction(p: wref<NPCPuppet>) -> Void {
  ...
  // Match the base AIHitReactionTask.Deactivate path instead of stopping only
  // the proxy action. Cyberpunk returns the NPC upper-body state to Normal and
  // then stops the active hit-reaction action.
  NPCPuppet.ChangeUpperBodyState(p, gamedataNPCUpperBodyState.Normal);
...
protected cb func OnRFC_HitReactionGuardEvent(evt: ref<RFC_HitReactionGuardEvent>) -> Bool {
  ...
  // V1714: ordinary hit-animation cutoff is owned directly by OnHitAnimation
  // so the cutoff clock begins at the real animation start.
  return true;
}
...
protected cb func OnRFC_HitReactionCutEvent(evt: ref<RFC_HitReactionCutEvent>) -> Bool {
  if !IsDefined(evt)
    || (evt.serial > 0 && evt.serial != this.m_RFC_HitAnimSerial)
  ...
  let cfg: RFCConfig = RFC.Cfg();
  if cfg.vanillaMode || RFC_TimeDilationBlocksImpulses(this, cfg) || cfg.hitReactionsDisabled || cfg.hitReactionCutoffDelay <= 0.001 {
    return true;
  }
...
private func RFC_SHHJM_ResolveOrDeadFallback(p: ref<NPCPuppet>, evt: ref<gameHitEvent>, hitPos: Vector4, s: ref<SHHJM_Settings>, out part: Int32, out boneIndex: Int32, out anchorPos: Vector4) -> Bool {
  let broadAnchor: Vector4;
  ...
  if SHHJM_ResolveBodyPartFromHitEvent(p, evt, hitPos, part, broadAnchor) {
    if SHHJM_ResolveExactBoneIndex(p, hitPos, part, boneIndex, anchorPos) {
      // Torso uses SPLAT's previously working broad-part model. Exact bone
      // anchors remain exclusive to head and limbs.
      if part == 1 {
        anchorPos = broadAnchor;
      };
      return true;
    };
    boneIndex = SHHJM_DefaultBoneIndexForPart(part);
    anchorPos = broadAnchor;
    return true;
  };

  // Spatial fallback for unusual/synthetic hit events with no hit-shape data.
  if SHHJM_ResolveBodyPart(p, hitPos, part, broadAnchor) {
    if SHHJM_ResolveExactBoneIndex(p, hitPos, part, boneIndex, anchorPos) {
      if part == 1 {
        anchorPos = broadAnchor;
      };
...
protected cb func OnHit(evt: ref<gameHitEvent>) -> Bool {
  ...
  shhjmTargetWasAlreadyDead = this.IsDead() || this.IsRagdolling();
  ...
  if shhjmRuntimeEnabled && !shhjmHardBlock && !RFC_IsVehicleContext(this) {
    if !this.shhjm_lastHitValid && (this.IsDead() || this.IsRagdolling()) {
      if RFC_SHHJM_ResolveOrDeadFallback(this, evt, hitPos, s, part, boneIndex, anchorPos) && SHHJM_GetPartEnabled(part, s) {
        this.shhjm_lastHitValid = true;
        this.shhjm_lastHitPos = hitPos;
        this.shhjm_lastSrcPos = shhjmSrcPos;
        this.shhjm_lastAnchorPos = anchorPos;
  ...
  // Ordinary hit-animation cutoff now lives inside OnHitAnimation itself.
  // This guard remains only for Arcade/Injury Shock scheduling.
  let allowAnimationCut: Bool = false;
  ...
  if allowArcadeImpulse || injuryShockQueued {
