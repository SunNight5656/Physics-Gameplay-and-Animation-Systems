...
import RealisticPush.{RFC, RFCConfig, RFCModSettings, SPLATSettingsRuntime, RFC_TimeDilationBlocksImpulses}
...
private func SGK_IsValidCorpseTarget(npc: wref<NPCPuppet>, s: ref<RFCModSettings>) -> Bool {
  if !IsDefined(npc) {
    return false;
  };
...
private func SGK_IsStickyNearbyValid(p: wref<PlayerPuppet>, npc: wref<NPCPuppet>, s: ref<RFCModSettings>) -> Bool {
  let ppos: Vector4;
  let npos: Vector4;
  let dist: Float;
...
private func SGK_GetPreferredCorpseTarget(p: wref<PlayerPuppet>, s: ref<RFCModSettings>) -> wref<NPCPuppet> {
...
private func SGK_IsGroundReady(p: wref<PlayerPuppet>, npc: wref<NPCPuppet>, s: ref<RFCModSettings>) -> Bool {
  let npos: Vector4;
  let id: EntityID;
...
protected cb func OnSGK_ApplyEvt(e: ref<SGK_ApplyEvt>) -> Bool {
  ...
  if IsDefined(e) && IsDefined(e.target) && RFC_TimeDilationBlocksImpulses(e.target, cfg) { return true; }
...
protected cb func OnSGK_ArmEvt(e: ref<SGK_ArmEvt>) -> Bool {
  let s: ref<RFCModSettings> = SPLATSettingsRuntime.Menu();
  let npc: wref<NPCPuppet>;
