...
private func RFC_TryRigSlotPos(puppet: ref<NPCPuppet>, slotName: CName, out pos: Vector4) -> Bool {
  let slotComponent: ref<SlotComponent>;
  let slotTransform: WorldTransform;

  if !IsDefined(puppet) { return false; }
  slotComponent = puppet.GetSlotComponent();
...
private func RFC_MidPoint(a: Vector4, b: Vector4) -> Vector4 {
  return new Vector4(
    (a.X + b.X) * 0.50,
    (a.Y + b.Y) * 0.50,
    (a.Z + b.Z) * 0.50,
...
public func RFC_BuildPositions(
  ...
  let leftHip: Vector4;
  let rightHip: Vector4;
  let gotLeftHip: Bool;
  let gotRightHip: Bool;

  // Preserve the old neutral-rig positions as fallbacks for unusual NPC rigs.
  ...
  // Use actual animated rig slots whenever the NPC exposes them. This keeps
  // Head, Chest, Pelvis, Knee, Shin, and Foot controls on their named regions.
  RFC_TryRigSlotPos(puppet, n"Head", headPos);
  if !RFC_TryRigSlotPos(puppet, n"Spine3", chestPos) {
    RFC_TryRigSlotPos(puppet, n"Chest", chestPos);
  }
  if !RFC_TryRigSlotPos(puppet, n"Hips", pelvisPos) {
    if !RFC_TryRigSlotPos(puppet, n"Pelvis", pelvisPos) {
      gotLeftHip = RFC_TryRigSlotPos(puppet, n"LeftUpLeg", leftHip);
      gotRightHip = RFC_TryRigSlotPos(puppet, n"RightUpLeg", rightHip);
      if gotLeftHip && gotRightHip { pelvisPos = RFC_MidPoint(leftHip, rightHip); }
    }
  }

  if !RFC_TryRigSlotPos(puppet, n"LeftLeg", leftKneePos) {
    RFC_TryRigSlotPos(puppet, n"LeftUpLeg", leftKneePos);
  }
  if !RFC_TryRigSlotPos(puppet, n"RightLeg", rightKneePos) {
    RFC_TryRigSlotPos(puppet, n"RightUpLeg", rightKneePos);
  }
  RFC_TryRigSlotPos(puppet, n"LeftLeg", leftShinPos);
  RFC_TryRigSlotPos(puppet, n"RightLeg", rightShinPos);
  if !RFC_TryRigSlotPos(puppet, n"LeftFoot", leftFootPos) {
    RFC_TryRigSlotPos(puppet, n"LeftToe", leftFootPos);
  }
  if !RFC_TryRigSlotPos(puppet, n"RightFoot", rightFootPos) {
    RFC_TryRigSlotPos(puppet, n"RightToe", rightFootPos);
  }
}
