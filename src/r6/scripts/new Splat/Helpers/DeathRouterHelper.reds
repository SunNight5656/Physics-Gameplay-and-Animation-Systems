...
public func RFC_Explode_Clear(p: wref<NPCPuppet>) -> Void {
  if !IsDefined(p) { return; }
  p.rfcExplodeUntil = 0.0;
}
...
@addMethod(NPCPuppet)
public func RFC_ClearRFCBlock() -> Void {
  this.rfc_blockUntil = 0.0;
}
...
protected cb func OnRFC_CutDeathAnimEvent(evt: ref<RFC_CutDeathAnimEvent>) -> Bool {
  ...
  if RFC_IsStealthOrFinisher(this) && !RFC_TimeDilationBlocksImpulsesNow(this) { return true; }
  if RFC_IsVehicleContext(this) { return true; }
  if this.IsRagdolling() { return true; }

  // The configured compatibility delay is the intentional handoff point for
...
public func RFC_ApplyHeadSlam(
  ...
  if c.vanillaMode || RFC_TimeDilationBlocksImpulses(p, c) { return; }
...
public func RFC_ApplyGlobalSettle(
  ...
  if c.vanillaMode || RFC_TimeDilationBlocksImpulses(puppet, c) { return; }
  if !c.settleEnabled || c.settleRadius <= 0.0 || c.settleStrength <= 0.0 {
    return;
  };
