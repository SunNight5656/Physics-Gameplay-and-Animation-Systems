...
// v1703 Vanilla Impulse + Death Animation rule:
// Each weapon toggle is authoritative for that weapon. The old separate master
// could leave every visible weapon toggle inert when it was OFF. Preserve an
// explicitly enabled master, but automatically activate the exception whenever
// at least one per-weapon Vanilla toggle is enabled.
if !c.vanillaMode {
  c.vanillaImpulsesEnabled =
    c.vanillaImpulsesEnabled
    || c.vanillaAllowHandgun
    || c.vanillaAllowMagnum
    || c.vanillaAllowShotgun
    || c.vanillaAllowSniper
    || c.vanillaAllowSMG
    || c.vanillaAllowAR
    || c.vanillaAllowLMG
    || c.vanillaAllowBlunt
