...
public final func BVCEnsureState() -> ref<BVCState> {
  ...
  state = new BVCState();
  state.activeMode = 0;
  // Debug/controller test bindings must never be active during normal play.
  state.debugPopups = false;
...
public func BVCForceCurrentRiderOff(
  ...
  // Motorcycle death-animation control. The ScriptedPuppet OnHit wrapper runs
  // after vanilla OnHit. If this rider has died and the kill switch is OFF,
  // BVC leaves the native mounted death-animation path alone.
  npcRider = rider as NPCPuppet;

  if IsDefined(npcRider) && npcRider.IsDead() {
    config = BVCGetActiveConfig(
      bike,
      mode
    );
    ...
    if IsDefined(config)
      ...
      BVCNotify(
        bike,
        sourceLabel
          + " | MOTORCYCLE DEATH ANIMATION PRESERVED"
      );
...
public func BVCTryBulletTopple(
  ...
  // Hard master gate. OFF also clears an unfinished bullet threshold.
  if !IsDefined(config)
    || !config.enabled {
  ...
  // Hard bullet-topple gate. OFF means bullet hits cannot trigger the custom
  // motorcycle topple path and no hidden partial count is retained.
  ...
  // Deterministic per-bike threshold. 1 = first valid bullet hit.
  ...
  // Preserve the existing chance/cooldown/topple actuator after the threshold.
...
private final func BVCRegisterInputs() -> Void {
  ...
  this.RegisterInputListener(this, n"Jump");
...
protected cb func OnAction(
  ...
  // Controller X / Jump is a debug-only rider-release test.
  // It intentionally does NOT topple the motorcycle. This separates rider
  // handoff testing from the Square manual-topple test.
  if Equals(actionName, n"Jump") {
    ...
    if !IsDefined(config)
      ...
      BVCNotify(
        this,
        "MANUAL X RIDER TEST BLOCKED: MODE DISABLED"
      );
      return result;
    };
    ...
    if !IsDefined(bike) {
      BVCNotify(
        this,
        "MANUAL X RIDER TEST: NO MOTORCYCLE OR RIDER"
      );
      return result;
    };
